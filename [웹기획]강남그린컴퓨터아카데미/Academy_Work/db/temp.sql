...
create trigger trg
    ...
    begin
		insert into test2 values ('abcd5', 'fdsa', 'asdf5');
    end $$
delimiter ;
update test set col2 = 'aaa' where col1 = 'abcd2';
delimiter $$
create trigger trg2
	before delete on test for each row
    begin
		insert into test2 values ('abcd7', 'fdsa', 'asdf5');
    end $$
delimiter ;
delete from test where col1 = 'abcd2';
...
-- test_cursor테이블에 20개의 데이터를 입력하는 프로시저 cp_cursor()를 제작
-- 입력받은 c2에 10씩 증가하는 값을 0부터 입력 / c3에는 '홍길동1' 부터 '홍길동20' 까지 입력
DROP procedure IF EXISTS cp_cursor;
DELIMITER $$
CREATE PROCEDURE cp_cursor()
BEGIN
	declare i int default 1;
    while i <= 20 do
		insert into test_cursor (c2, c3) values (i * 10 - 10, concat('홍길동',i));
        set i = i + 1;
    end while;
END$$
DELIMITER ;
...
BEGIN
	...
	declare continue handler for not found 
		set end_row = true; 				-- 커서의 끝에 닿으면 end_row를 true로 변경

	open c_set;								-- c_set이라는 커서를 열어줌
    
	lblLoop:while true do
		fetch c_set into cid; 				-- c_set 에 들어있는 데이터를 지정한 변수에 저장
...
-- sp_cursor를 참조하여 c1의 값이 홀수이면 c2의 값을 5 줄이고, 짝수이면 5 증가시키는 sp_cursor2 프로시저 작성 
...
BEGIN
	...
	lblLoop:while true do
        ...
        -- cid와 cid2가 짝수이면 레코드의 c2 컬럼의 값을 0으로 변경
