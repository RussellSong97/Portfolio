...
public class KISA_SEED_CBC {
	 ...
	 * @return
	 */
	public static String SeedEncryptPlaintext(String PlainText, byte KEY[], byte IV[]){
		byte[] Encrypt = null;
		String charset = "UTF-8";
		String resultCipherText = "";
		try {
			/* 암호화 시작 */
			Encrypt = SEED_CBC_Encrypt(KEY, IV,
					PlainText.getBytes(charset), 0, PlainText.getBytes(charset).length);
			resultCipherText = byteArrayToHex(Encrypt);
	 ...
	 * @return
	 */
	public static String SeedDecryptPlaintext(String CipherText, byte KEY[], byte IV[]) throws UnsupportedEncodingException{
		byte[] Decrypt = null;
		byte[] CipherData = hexToByteArray(CipherText);
		String resultPlainText = "";
		/* 복호화 시작 */
		Decrypt = SEED_CBC_Decrypt(KEY, IV, CipherData, 0, CipherData.length);
		resultPlainText = new String(Decrypt, "utf-8");
