...
public class AttachmentService {
    ...
    public List<Attachment> uploadFilesInfo(List<MultipartFile> files,
                                            String source,
        ...
        for (MultipartFile file : files)
            try {
                attachments.add(this.uploadFileInfo(file, source, request, authentication));
            } catch (Exception e) {
                log.error(e.getMessage());
                e.printStackTrace();
            }
    ...
    private Attachment uploadFileInfo(MultipartFile file, String source, HttpServletRequest request, Authentication authentication) throws IOException {
        // 랜덤 UUID 생성
        UUID uuid = UUID.randomUUID();
        // 월별로 구분 (예: 2022/08)
        String splitDir = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyy/MM"));
        ...
        member.setProfileImageJson(this.findUploadFileDtoByUUID(attachment.getUuid()));
