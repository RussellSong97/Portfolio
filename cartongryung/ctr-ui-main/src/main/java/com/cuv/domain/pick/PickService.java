...
public class PickService {
     ...
     * @author 송다운
     */
    public Long searchPicksMemberIdReadYn(UserDetails auth) {
        if (auth == null) {
            return 0L;
        } else {
            String loginId = auth.getUsername();
    ...
    /**
          ...
          * @autor 송다운
          */
    public JSONResponse<?> insertPickV2(Map<String, Object> map, UserDetails userDetails) {
        if (userDetails == null) {
            return new JSONResponse<>(500, "로그인이 필요합니다.");
        }
    ...
    public List<YN> getDelYNOfPick(Long productId) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String email = authentication.getName();
        Member member = memberRepository.findByEmail(email); // 이메일로 맴버 객체 가져옴
