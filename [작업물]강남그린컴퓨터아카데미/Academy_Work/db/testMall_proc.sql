...
begin
insert into t_admin_menu(am_idx, am_name, am_link, am_parent, am_seq, ai_idx) values(amidx, amname, amlink, amparent, amseq, aiidx);
...
delimiter $$
create procedure sp_ad_pms_insert(apaiidx int, amix int, appms int, aiidx int)
begin
...
delimiter ;
call sp_ad_pms_insert(1,'127.0.0.1');
select * from t_admin_login_log;
...
create procedure sp_member_update(miid varchar(20), miphone varchar(13), 
	miemail varchar(50), miissns char(1), miismail char(1), mirebank varchar(50), miaccount varchar(50) )
begin
	update t_member_info
    set mi_phone = miphone, mi_email = miemail, mi_issns = miissns, 
		mi_ismail = miismail, mi_rebank = mirebank, mi_account = miaccount
	where mi_id = miid;
end $$
delimiter ;
call sp_member_update('test3', '010-1234-4444', 'dooly@naver.com', 'y', 'n', 'SC은행', '32165-12-123');
select * from t_member_info;
...
begin
	update t_member_info set miisactive = miisactive  where mi_id = miid;
end $$
delimiter ;
call sp_member_out('test2', 'n');
select * from t_member_info;

ALTER TABLE `testmall`.`t_member_point` 
ADD COLUMN `ai_idx` INT NULL DEFAULT 0 COMMENT '관리자번호' AFTER `mp_date`,
CHANGE COLUMN `mp_kind` `mp_kind` CHAR(1) NULL DEFAULT 's' COMMENT '사용/적립 여부\ns: 적립(주문)\nr: 적립(후기)\na: 적립(관리자)\nu:사용' ;


-- 회원 포인트 관리자 직접 입력 프로시저
drop procedure if exists sp_point_insert;
delimiter $$
create procedure sp_point_insert(miid varchar(20), mppoint int, mplinknum varchar(10), 
								mpcontent varchar(50), aiidx int)
begin
...
delimiter ;
call sp_point_insert('test4', 1500, '', '관리자갑질', 1 );
...
begin
    ...
    set ma_zip = mazip,  ma_addr1 = maaddr1,  ma_addr2 = maaddr2,  ma_basic = mabasic
    where ma_idx = maidx and mi_id = miid; 
end $$
delimiter ; 
...
delimiter $$
create procedure sp_pdt_cata_insert(kind char(1), cbid char(2), cbname varchar(20), 
									csid char(4), csname varchar(20))
begin 
...
delimiter $$
create procedure sp_pdt_cata_update(kind char(1), cbid char(2), cbname varchar(20), 
									csid char(4), csname varchar(20))
begin 
...
create procedure sp_product_insert(
	piid char(6), piname varchar(20), csid char(5), bid char(4), piorigin varchar(20), 
	picost int, piprice int, pidiscount int, pioption varchar(100), 
	piimg1 varchar(50), piimg2 varchar(50), piimg3 varchar(50), pidesc varchar(50), 
	pistock int, piisview char(1), aiidx int)
begin    
	insert into t_product_info (pi_id, pi_name, cs_id, b_id, pi_origin, pi_cost, pi_price, pi_discount, pi_option, 
								pi_img1, pi_img2, pi_img3, pi_desc, pi_stock, pi_isview, ai_idx)
...
call sp_product_insert('pdt021', '술20', 'B2S03', 'BR01', '대한민국', 33000, 40000, 0, '45도;50도;60도', 
'pdt021_1.jpg', '','', 'pdt021_d.jpg', 1500, 'y', 1 );
...
create procedure sp_product_update(
	...
	picost int, piprice int, pidiscount int, pioption varchar(100), 
	piimg1 varchar(50), piimg2 varchar(50), piimg3 varchar(50), pidesc varchar(50), 
	pistock int, piisview char(1))
begin    
	update t_product_info set
		pi_id = piid, pi_name = piname, cs_id = csid, b_id = bid, 
        pi_origin = piorigin, pi_cost = picost, pi_price = piprice, pi_discount = pidiscount, pi_option = pioption, 
		pi_img1 = piimg1, pi_img2 = piimg2, pi_img3 = piimg3, pi_desc = pidesc, pi_stock = pistock, pi_isview = piisview
	where pi_id = piid;
end $$
delimiter ; 

call sp_product_update('pdt021', '술21', 'B2S03', 'BR01', '대한민국', 33000, 40000, 0, '45도;50도;60도', 
'pdt021_1.jpg', '','', 'pdt021_d.jpg', 1500, 'y' );
...
delimiter $$
create procedure sp_ad_notice_insert(bntitle varchar(100), bncontent text, bnisnotice char(10), aiidx int)
begin    
	...
	if bnisnotice = 'y' then -- 현재 등록하려는 글을 공지글로 등록한다면
        ...
        if cnt >= 3 then -- 현재 공지글의 개수가 3이상이면 (최대 3개이므로 하나를 일반글로 변경해야 함)
...
insert into t_brd_notice (bn_title, bn_content, bn_isnotice, ai_idx) values ('1번째 공지사항입니다.', '내용111', 'n', 1) ;
insert into t_brd_notice (bn_title, bn_content, bn_isnotice, ai_idx) values ('2번째 공지사항입니다.', '내용222', 'n', 1) ;
insert into t_brd_notice (bn_title, bn_content, bn_isnotice, ai_idx) values ('3번째 공지사항입니다.', '내용333', 'n', 1) ;
insert into t_brd_notice (bn_title, bn_content, bn_isnotice, ai_idx) values ('4번째 공지사항입니다.', '내용444', 'n', 1) ;
insert into t_brd_notice (bn_title, bn_content, bn_isnotice, ai_idx) values ('5번째 공지사항입니다.', '내용555', 'n', 1) ;
insert into t_brd_notice (bn_title, bn_content, bn_isnotice, ai_idx) values ('6번째 공지사항입니다.', '내용666', 'n', 1) ;
insert into t_brd_notice (bn_title, bn_content, bn_isnotice, ai_idx) values ('7번째 공지사항입니다.', '내용777', 'y', 1) ;
insert into t_brd_notice (bn_title, bn_content, bn_isnotice, ai_idx) values ('8번째 공지사항입니다.', '내용888', 'y', 1) ;
insert into t_brd_notice (bn_title, bn_content, bn_isnotice, ai_idx) values ('9번째 공지사항입니다.', '내용999', 'y', 1) ;


-- 공지사항 등록 프로시저
drop procedure if exists sp_ad_notice_update;
delimiter $$
create procedure sp_ad_notice_update(bnidx int, bntitle varchar(100), bncontent text, bnisnotice char(1))
begin
	declare cnt int;
	declare idx int;
	set cnt = 0;
	...
	if bnisnotice = 'y' then
		...
		from t_brd_notice where bn_isnotice = 'y'and bn_idx <> bnidx;
	...
	update t_brd_notice set bn_title = bntitle, bn_content = bncontent, bn_isnotice = bnisnotice
    where bn_idx = bnidx;
end $$
delimiter ;
call sp_ad_notice_update(7, '7째 공지사항입니다.', '내용777', 'y');
select * from t_brd_notice; 
...
delimiter ;  
call sp_ad_notice_delete(6);


-- FAQ 등록 프로시저
drop procedure if exists sp_ad_faq_insert;
delimiter $$
create procedure sp_ad_faq_insert(bfcafe varchar(20), bftitle varchar(100), bfcontent text, aiidx int)
begin      
...
delimiter $$
create procedure sp_ad_faq_update(bqidx int, bfcafe varchar(20), bftitle varchar(100), bfcontent text)
begin     
	update t_brd_faq set bf_cata = bfcata, bf_title = bftitle, bf_content = bfcontent where bq_idx = bqidx;
end $$
delimiter ;  
...
delimiter $$
create procedure sp_ad_faq_delete(bqidx int)
begin     
	delete from t_brd_faq where bq_idx = bqidx;
end $$
delimiter ;  
...
delimiter $$
create procedure sp_qna_insert(miid varchar(20), bqcata varchar(20), bqtitle varchar(100), bqcontent text, 
bgimg varchar(50))
begin      
	insert into  t_brd_qna (mi_id, bq_cata, bq_title, bq_content, bg_img)
    values (miid, bqcata, bqtitle, bqcontent, bgimg);
end $$
delimiter ;  
call sp_qna_insert('test1', '회원 관련', '질문있어요', '이게 뭔데??', null);
...
delimiter $$
create procedure sp_qna_update(bqidx int, bqcata varchar(20), bqtitle varchar(100), bqcontent text, bgimg varchar(50))
begin     
	update t_brd_qna
    set bq_cata = bqcata, bq_title = bqtitle, bq_content = bqcontent, bg_img = bgimg 
    where bq_idx = bqidx and bq_status = 'a';
    -- 질문에 대한 수정으로 관리자가 답변을 달기 전에만 수정이 가능함 
end $$
delimiter ;  
call sp_qna_update(3, '회원 관련', '회원관련 질문있어요333', '대답해주실수잇나요333', 'qna.jpg');
 
 
-- QnA 답변/수정 프로시저II(관리자용)
...
-- QnA 수정 프로시저 (관리자 대답)
drop procedure if exists sp_ad_qna_update;
delimiter $$
create procedure sp_ad_qna_update(bqidx int, bqanswer text, aiidx int)
begin     
	update t_brd_qna
    set bq_answer = bqanswer, ai_idx = aiidx, bq_adate = now(), bq_status = 'b' 
    where bq_idx = bqidx and bq_status = 'a';
    -- 질문에 대한 수정으로 관리자가 답변을 달기 전에만 수정이 가능함 
end $$
delimiter ;  
call sp_ad_qna_update(3, '말하거라', 1);
...
ALTER TABLE `testmall`.`t_member_point` 
ADD COLUMN `br_idx` INT NULL DEFAULT 0 AFTER `mp_point`;
ALTER TABLE `testmall`.`t_member_point` 
DROP COLUMN `mp_linknum`;
ALTER TABLE `testmall`.`t_brd_review` 
ADD COLUMN `br_opt` VARCHAR(10) NULL DEFAULT '' COMMENT '선택한 옵션' AFTER `pi_id`;
ALTER TABLE `testmall`.`t_brd_review` 
CHANGE COLUMN `br_opt` `br_opt` VARCHAR(10) NOT NULL DEFAULT '' COMMENT '선택한 옵션' ,
DROP PRIMARY KEY,
ADD PRIMARY KEY (`mi_id`, `oi_id`, `pi_id`, `br_opt`);
;
...
begin
	...
	insert into t_brd_review (mi_id, oi_id, pi_id, br_title, br_content, br_img1, br_img2, br_star) values (miid, oiid, piid, brtitle, brcontent, brimg1, brimg2, brstar);
    ...
    -- 주문상세 테이블 : t_order_detail				
    select format(((od_pdtprice * od_cnt) * 0.01), 0) into price from t_order_detail where oi_id = oiid and pi_id = piid;
    update t_order_detail set od_saveptn = price where oi_id = oiid and pi_id = piid; 

    -- 회원 정보 테이블 : t_member_info				
    update t_order_detail set mi_point = mi_point + price where mi_id = miid; 
    
    -- 포인트 사용내역 테이블 : t_member_point				
	insert into t_member_point (mi_id, mp_kind, mp_point, mp_linknum, mp_content) 
	values (miid, 's', price, oiid, concat(oiid,'후기등록'));
...
ALTER TABLE `testmall`.`t_brd_review_replay` 
CHANGE COLUMN `brr_recc` `brr_recc` INT NULL DEFAULT '0' COMMENT '추천수' ;
...
-- 주문번호도 프로시저 안에서 생성해야 하나, 프로시저가 아닌 자바에서 쿼리 작업시에는 자바에서 주문번호도 만들어야 함
-- 바로구매(ocidx : 0), 카트구매 - 한가지만 구매 & 여러 가지 구매 (ocidxs : 5, 6, 7)
drop procedure if exists sp_order_insert;
delimiter $$
create procedure sp_order_insert(miid varchar(20), oiname varchar(20), oiphone varchar(13), oizip char(5), oiaddr1 varchar(50), oiaddr2 varchar(50), oipayment char(1), 
...
select a.pi_id, a.oc_option, a.oc_cnt, b.pi_name, b.pi_img1, b.pi_price from t_order_cart a, t_product_info b
where a.pi_id = b.pi_id and a.oc_idx in ('5', '6', '7');

ALTER TABLE `testmall`.`t_order_info` 
ADD COLUMN `oi_modify` DATETIME NULL DEFAULT now() COMMENT '마지막 수정일자' AFTER `oi_date`,
ADD COLUMN `ai_admin` INT NULL DEFAULT 0 COMMENT '관리자번호\n' AFTER `oi_modify`;
...
delimiter $$
create procedure sp_all_order_update(aiidx int, miid varchar(20), oiid char(10), oistatus char(1), oiname varchar(20), oiphone varchar(13), oizip char(5), oiaddr1 varchar(50), oiaddr2 varchar(50))
begin
